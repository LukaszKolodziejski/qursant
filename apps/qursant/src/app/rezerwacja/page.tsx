...
const validateName = (name: string): { isValid: boolean; isSpam?: boolean } => {
  ...
  // Realnie najdłuższe spotykane nazwiska mieszczą się w tym limicie
  // (np. "Brzęczyszczykiewicz" ma 19 znaków).
  ...
  for (const word of words) {
    ...
    // Słowo nie może zaczynać/kończyć się myślnikiem lub apostrofem,
    // ani zawierać dwóch separatorów obok siebie (np. "--").
    if (/^[-']|[-']$|[-']{2,}/.test(word)) {
      return { isValid: false, isSpam: true };
    }
    // Każda część (rozdzielona '-' lub "'") musi mieć rozsądną długość
    // i przynajmniej jedną samogłoskę — to odsiewa zlepki klawiaturowe
    // typu "fsdlfdsfdsdfksdlfjsldkjsklfjlsdjljll".
  ...
  //    (np. "Kowalski", "Nazwisko-Nazwisko", "Nazwisko-nazwisko", "O'Brien", "D'angelo")
  const isAllLowerWord = (word: string) => word === word.toLowerCase();

  const isProperCaseWord = (word: string) => {
    // Dzielimy słowo na podsłowa po myślnikach i apostrofach.
    // Pierwsza część musi zaczynać się wielką literą; kolejne mogą zaczynać
    // się wielką lub małą (np. "Nowak-Kowalski" lub "Nowak-kowalski").
    // W każdej części pozostałe litery muszą być małe.
